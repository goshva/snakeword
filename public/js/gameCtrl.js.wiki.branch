...
function SaveGame(){
var link = "https://boggle.space/url"
               +'?'+"edge="+edge
...
function ok(data) {
  ...
  if (data["query"]["pages"][pageId[0]]['images']){
    var picName =  data["query"]["pages"][pageId[0]]['images'][0]['title'].substring(5)
    if (picName ==  "Commons-logo.svg" || picName =="Disambig.svg"){ console.log("default"); data["query"]["pages"][pageId[0]]['images'].splice(0,1); var picName =  data["query"]["pages"][pageId[0]]['images'][0]['title'].substring(5)}
